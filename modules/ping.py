...
supereffective.rule = r'ACTION uses (Force Palm|Aura Sphere|Close Combat|Focus Blast|Brick Break) on (I|Bli|Pi)nkie(Pie)?(Bot)?'
...
noteffective.rule = r'ACTION uses Psychic on (I|Bli|Pi)nkie(Pie)?(Bot)?'
...
splash.rule = r'ACTION uses Splash on (I|Bli|Pi)nkie(Pie)?(Bot)?'
...
tailwhip.rule = r'ACTION uses Tail Whip on (I|Bli|Pi)nkie(Pie)?(Bot)?'
...
healpulse.rule = r'ACTION uses Heal Pulse on (I|Bli|Pi)nkie(Pie)?(Bot)?'
...
flirt.rule = r'(?i)(ACTION uses (Attract|Captivate) on (I|Bli|Pi)nkie(Pie)?(Bot)?)'
# TODO: add more rules
...
attacked.rule = r'ACTION uses (Dragon Pulse|Metal Claw|ExtremeSpeed|Quick Attack) on (I|Bli|Pi)nkie(Pie)?(Bot)?'
